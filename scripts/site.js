...
function loadApplication() {
  ...
  if (la != undefined) {
    ...
    // NOT an array on LHS since it is to left of =
    const [
      isEmployed,
      hasKids = true, // default
      hasLoans,
      hasCreditcards,
      ...moreArgs
    ] = la.Factors;

    // la.ApplicantName = undefined;

    // deconstruct object only for demo here but its possible
    const { Id: a, ApplicantName: b = "Barry" } = la;
    console.log(a, b);
    ...
    document.getElementById(
      "inputDoBDay"
    ).value = la.ApplicantDateOfBirth.getDate();
    document.getElementById(
      "inputDoBYear"
    ).value = la.ApplicantDateOfBirth.getFullYear();
...
function generateRickProfile(la) {
  ...
  const dr = nameAndTitle.startsWith("dr");
  const phd = nameAndTitle.startsWith("phd");
  const phd2 = nameAndTitle.startsWith("ph.d");
  const md = nameAndTitle.endsWith("md");
  const md2 = nameAndTitle.endsWith("m.d");
  const md3 = nameAndTitle.endsWith("m.d.", 4);
  ...
  const house = purpose.includes("house");
  const holiday = purpose.includes("holiday");
  const vacation = purpose.includes("vacation");
  const business = purpose.includes("business");
  ...
  const applicationCode = String.raw`\t${createApplicationId()}`;
